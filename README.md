...
```bash
my-application$ sam local invoke renderWidgetFunction --event events/event-post-item.json
```
...
```bash
my-application$ sam logs -n renderWidgetFunction --stack-name sam-app --tail
```
...
```bash
aws cloudformation delete-stack --stack-name serverless-widget-render
```
