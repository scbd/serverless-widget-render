...
const { isRegExp, keys, cloneDeep, filter, each, isString, isObject } = require('lodash');
...
exports.renderHandler = async (event) => {
    ...
    try{
        ...
        if((widget.dataSource||[]).length){
            for(const key in widget.dataSource){
                const source = widget.dataSource[key]
                if(source.queryString)
                    queryStringValidators   = {...queryStringValidators , ...source.queryString }
            }
        }
    ...
    console.debug(`response from: ${event.path} statusCode: ${response.statusCode} body: ${response.body}`);
...
const validateWidget = (widget)=>{
    ...
    if((widget.template||'').trim() == '')
        throw new HttpError(403, 'Please provide widget template');

    if((widget.contentType||'').trim() == '')
        throw new HttpError(403, 'Please provide widget contentType');

    if(widget.dataSource){
        for (let i = 0; i < widget.dataSource.length; i++) {
            const dataSource = widget.dataSource[i];            
            if((dataSource.name||'').trim() == '')
                throw new HttpError(403, 'Please provide widget data source name');
            
            if((dataSource.url||'').trim() == '')
                throw new HttpError(403, 'Please provide widget data source url');
                
            if((dataSource.method||'').trim() == '')
                throw new HttpError(403, 'Please provide widget data source HTTP method');

            if(dataSource.queryString){
                for(const key in dataSource.queryString){
            ...
            if(dataSource.formData){
                for(const key in dataSource.formData){
                    const qs = dataSource.queryString[key]
                    if(qs.type == 'regex'){
                        try{
                            stringToRegex(qs.validationRegex);
                        ...
                        const schemaValidation = validateJsonSchema(qs.validationJsonSchema);
                        if(schemaValidation.errors.length){
                            const errors = _.map(schemaValidation.errors, ({message, type}) => ({
                                                message: message.replace(/['"]/g, ''),
                                                type
                                            }));
                            throw new HttpError(403, errors);
...
const validateParams = (params, validators, type)=>{
    ...
    if(missingParams.length)
        throw new HttpError(403, `The widget requires these params to proceed, "${missingParams.join('", "')}"`);
...
const fetchDataSource = async ($qsSource, $formSource, dataSource)=>{
    ...
    for (const key in dataSource) {
        if (dataSource.hasOwnProperty(key)) {
            ...
            for (const param in source.queryString) {
            ...
            for (const param in source.formData) {
            ...
            const normalizeResult = (result)=>{                    
                if(result && result.status == 200){
                    return { name:source.name, source : result.body };
                }
            };
            let queryPromise;
            if(source.method == 'GET'){
                queryPromise = request.get(query.url).query(query.params).then(normalizeResult);
            }
            else{
                queryPromise = request.post(query.url).query(query.params).send(query.formData).then(normalizeResult);
            }
    ...
    promiseResult.forEach(result=>{
        if(result)
            dataSourceResult[result.name] = result.source;
    })
    // 
